...
# Rename percentile columns so they don't clash with financial columns
rename_dict = {}

for col in percentile_df.columns:
    if col != "company_id":
        rename_dict[col] = f"{col}_pctile"

percentile_df.rename(columns=rename_dict, inplace=True)
...
for sector in sectors:
    ...
    row["return_on_equity_pct_pctile"],

    row["debt_to_equity"],
    row["debt_to_equity_pctile"],

    row["free_cash_flow_cr"],
    row["free_cash_flow_cr_pctile"],

    row["revenue_cagr_5yr"],
    row["revenue_cagr_5yr_pctile"],

    row["pat_cagr_5yr"],
    row["pat_cagr_5yr_pctile"],

    row["operating_profit_margin_pct"],

    row["interest_coverage"],
    row["interest_coverage_pctile"],

    row["asset_turnover"],
    row["asset_turnover_pctile"],
    ...
    row["composite_quality_score"]
        ...
        for col_idx, value in enumerate(values, start=1):
            ...
            if isinstance(value, (int, float)):
                if col_idx in [6, 8, 10, 12, 14, 17, 19]:
                    if value >= 0.75:
